...
import { Award, GraduationCap, Globe2, HeartHandshake, Linkedin, MapPin, Search, Share2 } from 'lucide-react';
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-zinc-200">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-100 text-zinc-700 text-xs font-semibold border border-zinc-200 shadow-2xs">
        ...
        {filteredAlumni.length === 0 ? (
          <div className="text-center py-16 px-4 rounded-3xl border border-dashed border-zinc-300 bg-white space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-6">
            {filteredAlumni.map((alum) => (
              <div key={alum.id} className="p-6 rounded-3xl bg-white border border-zinc-200 shadow-2xs hover:shadow-xs transition-all duration-200 flex flex-col justify-between group space-y-4 relative">
                <div className="space-y-4 text-center">
                  <div className="relative w-20 h-20 mx-auto rounded-full overflow-hidden border-2 border-terracotta-200 shadow-2xs group-hover:scale-105 transition-transform">
                    ...
                    {alum.mentorAvailable && (
                      <span title="Tersedia untuk Sesi Mentoring" className="absolute bottom-0 right-0 w-4 h-4 bg-emerald-500 border-2 border-white rounded-full" />
                    )}
                  </div>

                  <div className="space-y-1">
                    <h3 className="text-base font-bold text-zinc-900 font-display group-hover:text-terracotta-800 transition-colors">{alum.name}</h3>
                    <div className="text-xs font-semibold text-terracotta-700">{alum.role}</div>
                    <div className="text-xs text-zinc-600 font-medium">{alum.company}</div>
                ...
                <div className="space-y-2 pt-2 border-t border-zinc-100 text-center">
                  <div className="inline-block px-2.5 py-0.5 rounded-full bg-white text-[11px] font-semibold text-zinc-700 border border-zinc-200">
                    {alum.batch}
                  </div>
                  <p className="text-[11px] text-zinc-500 italic line-clamp-3 leading-relaxed">&ldquo;{alum.quote}&rdquo;</p>
                </div>

                <div className="flex flex-wrap items-center justify-center gap-1.5 pt-1">
                  {alum.skills.map((skill, sIdx) => (
                    <span key={sIdx} className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-terracotta-50 text-terracotta-800 border border-terracotta-100">
                      {skill}
                    </span>
                  ))}
                </div>
                ...
                <div className="pt-3 border-t border-zinc-100 flex items-center justify-between">
                  <button onClick={() => handleCopyProfile(alum)} title="Salin ringkasan profil" className="text-zinc-400 hover:text-zinc-600 p-1 rounded-md transition-colors">
                    <Share2 className="w-3.5 h-3.5" />
                  </button>
                  <a href={alum.linkedin} target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-zinc-700 hover:text-terracotta-700 flex items-center gap-1.5 transition-colors">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
              <Link href="/partnership">
                <Button variant="outline" className="w-full sm:w-auto border-white text-white bg-transparent hover:bg-white hover:text-zinc-900 shadow-sm">
                  Ajukan Kemitraan Daerah
                </Button>
              </Link>
              <Link href="/internship">
                <Button variant="outline" className="w-full sm:w-auto bg-white text-zinc-900 border-white hover:bg-zinc-50">
